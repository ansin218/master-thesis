...
Document Term Matrix converts the entire dataset in the form of individual words occurring in a comment column wise and the document, in this case, the comment itself, in a row wise fashion. A simple example illustrating the DTM for three comments listed below.
\newline

Comment 1: I learn machine learning 

Comment 2: You learn machine learning 

Comment 3: We love machine learning 
\begin{table}[h]
\caption{Document Term Matrix for the sample comments }
\captionsetup{width=6cm}
\label{my-label}
\begin{tabular}{|l|l|l|l|l|l|l|l|}
\hline
          & I & learn & machine & learning & we & you & love \\ \hline
Comment 1 & 1 & 1     & 1    & 0         & 0  & 0   & 0    \\ \hline
Comment 2 & 0 & 1     & 1    & 1         & 0  & 1   & 0    \\ \hline
Comment 3 & 0 & 0     & 1    & 1         & 1  & 0   & 1    \\ \hline
\end{tabular}
\end{table}
\bigbreak
After having obtained the DTM for the set of comments, constraints are applied to the same. To do so, only words existing in the English dictionary are taken into consideration. The next step is to consider the part of speech in English. The parts of speech consists of: nouns, verbs, pronouns, prepositions, adverbs, article, adjectives, conjunctions and interjections. Out of the nine entities, only certain nouns and verbs are of primary interest. Tf-idf is the product of two values: term frequency and inverse document frequency; it is illustrated as follows:
\begin{equation*}
\label{eq:et}
    tfidf(t,d,D) = tf(t,d).idf(t,D)
\end{equation*}
Term frequency can be defined as the number of times a term occurs in a document. On the other hand, inverse document frequency looks for strength of frequency for a given word, whether it is common or not. It can also be defined as the logarithmically scaled inverse fraction of the documents that contain the word, obtained by dividing the total number of documents by the number of documents containing the particular pertinent term, and then taking the logarithm (base 10) of that quotient. Consider the example used earlier in this section to form the dtm, to find out the tf-idf of the word "learn":
The formula for tf is as follows:
\begin{equation}
\label{eq:et}
    tf(t,d) = n
\end{equation}
Since the word "learn" occurs twice across the comments, 
\begin{equation}
\label{eq:et}
    tf(learn,d) = 2
\end{equation}
...
\end{equation}
The idf for the word "learn" will be:
\begin{equation}
\label{eq:et}
    idf(learn,D) = log\frac{3}{2} = 0.18
\end{equation}
Therefore, the tfidf is:
\begin{equation}
\label{eq:et}
    tfidf(learn,d,D) = 3\times0.18 = 0.54
\end{equation}
This tfidf is one of the constraints used to obtain the final matrix using which the word cloud is formed. The final matrix comprises of the terms and their respective count. Based on number of words to be chosen for the word cloud, the top \textit{n} number of words are taken into account for forming the cloud.
...
Decision tools predominantly used for classification and regression are decision trees. At the top most level, there exists one node from where decision needs to be made at every level going downwards based on some rules pertaining to the attribute in consideration, until the final goal is reached. 
\bigbreak
At every level, for each node, there exists an attribute that is chosen to split into their pertinent classes based on the samples provided. This split is done using a goodness function; some of them are information gain, information gain ratio and gini index, to mention a few. The condition for an attribute to be chosen is the one that gives the smallest tree and hence, an attribute that gives the purest node is chosen. A typical criteria for computing impurity is information gain which increases as the average value of purity of the relevant subsets an attribute produces. Attribute that results in highest information gain is therefore chosen finally. 
\bigbreak
Information Gain = (information before split)–(information after split)
\bigbreak
To compute the information required, the entropy is computed; entropy describes the randomness of a given system. 
\bigbreak
NOTE: Entropy formula goes here
\bigbreak
The highest information gain is chosen after computing the information gain for each of the attribute.
\bigbreak
NOTE: Gain formula goes here
\bigbreak
Splitting process occurs repeatedly until the scenario where data cannot be split any further. Later, from the bottom, pruning of the tree takes place to make it shorter and avoid overfitting of any kind. 
\bigbreak
NOTE: Add algorithm maybe?

\subsubsection{Random Forest}
Random forests belong to a special class of learning algorithms known as the ensemble learning algorithm. This methodology believes individual classifiers are weak and are not having high accuracy. Hence, random forests use techniques such as bagging to overcome this limitation. Random forests thus creates different subsets from the dataset and decision tree classifier is run on each of these subsets. Bagging is then used to combine the results of all the individual classifier run on different subsets to perform the final classification; it decreases the test error by lowering the variance of prediction. 
\bigbreak
Due to their nature of running the same algorithm multiple times over different subsets, random forests can be completely parallelized as well. Random forests have more advantages since it aggregates many weak classifiers to make the final classification. Thanks to its ensemble learning approach, it prevents overfitting and gives better accuracy. In addition to handling data without any kinds of preprocessing, random forests are also considered to be resitant to outliers and over-training. 
\bigbreak
Note: Add algorithm may be?
\bigbreak
Note: Add previous works
\bigbreak
Note: Add image may be?
