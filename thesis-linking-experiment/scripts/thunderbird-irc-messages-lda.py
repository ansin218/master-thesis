...
cursor_1.execute("SELECT DISTINCT(id), message FROM `thunderbird_binary`")

thunderbird_irc_list = list()
irc_msg_id_list = list()
keywords_list = list()

for row in cursor_1:
    irc_msg_id_list.append(row[0])
    thunderbird_irc_list.append(row[1])

for x in range(len(thunderbird_irc_list)):
	...
	customList31 = ['must', 'https', 'http', 'shouldn', 'shouldnt', 'really', 'supported', 'return', 'isnt', 'isn', 'busy', 'make', 'always', 'similar', 'dealt', 'behind', 'opnion', 'decent', 'directly', 'please', 'reply', 'replying', 'replied']
	...
	temp_list = list()
	temp_list.append(thunderbird_irc_list[x])
...
for a in range(len(thunderbird_irc_list)):
	try:
	    cursor_2.execute("""INSERT INTO thunderbird_irc_keywords_lda (msg_id, message, keywords) VALUES ("%s", "%s", "%s")""" % (irc_msg_id_list[a], thunderbird_irc_list[a], keywords_list[a]))
	except:
	    conn.rollback()
