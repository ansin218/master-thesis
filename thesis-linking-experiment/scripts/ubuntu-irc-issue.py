...
cursor_1.execute("SELECT issue_id, comment_id, keywords FROM ubuntu_issues_keywords_lda")

cursor_2 = conn.cursor()
cursor_2.execute("SELECT msg_id, keywords FROM ubuntu_irc_keywords_lda")
		...
		for y in range(len(irc_keyword_list)):
			if(irc_keyword_list[y] != 'Insufficient Data'):
				...
				if(z > 50):
					...
					print('\n')
					try:
					    cursor_3.execute("""INSERT INTO ubuntu_issue_irc (issue_id, comment_id, irc_msg_id, similarity) VALUES ("%s", "%s", "%s", "%s")""" % (issue_id_list[x], comment_id_list[x], irc_id_list[y], z))
					except:
					    conn.rollback()
