...
cursor_1.execute("SELECT DISTINCT(comment), comment_id, issue_id FROM `ubuntu_rss_comments` WHERE 1")

ubuntu_rss_comments = list()
keywords_list = list()
comment_id_list = list()
issue_id_list = list()

for row in cursor_1:
    ubuntu_rss_comments.append(row[0])
    comment_id_list.append(row[1])
    issue_id_list.append(row[2])

for x in range(len(ubuntu_rss_comments)):
	...
	customList34 = ['let', 'experiencing', 'visited', 'importance', 'information', 'give', 'gives', 'gave', 'expect', 'expected', 'expects', 'axtually', 'following', 'common', 'avoid', 'sorted']
	...
	temp_list = list()
	temp_list.append(ubuntu_rss_comments[x])
...
for a in range(len(ubuntu_rss_comments)):
	print('Comment ', a + 1, ' with issue_id ', issue_id_list[a], ' and comment_id ' , comment_id_list[a],' has ', keywords_list[a])
	try:
	    cursor_2.execute("""INSERT INTO ubuntu_issues_keywords_lda (comment_id, issue_id, comment, keywords) VALUES ("%s", "%s", "%s", "%s")""" % (comment_id_list[a], issue_id_list[a], ubuntu_rss_comments[a], keywords_list[a]))
	except:
	    conn.rollback()
