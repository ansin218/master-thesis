...
for row in cursor_1:
    ...
    ubuntu_irc_list.append(row[1])

for x in range(len(ubuntu_irc_list)):
	...
	temp_list = list()
	temp_list.append(ubuntu_irc_list[x])
...
for a in range(len(ubuntu_irc_list)):
	try:
	    cursor_2.execute("""INSERT INTO ubuntu_irc_keywords_lda (msg_id, message, keywords) VALUES ("%s", "%s", "%s")""" % (irc_msg_id_list[a], ubuntu_irc_list[a], keywords_list[a]))
	except:
	    conn.rollback()
